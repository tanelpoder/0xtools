...
// MD5 internal function declarations
...
// Main stack hashing function that uses MD5
uint64_t hash_stack(uint64_t *stack, int stack_len) {
    ...
    MD5_Init(&context);
    MD5_Update(&context, (const unsigned char*)stack, stack_len * sizeof(uint64_t));
    MD5_Final(digest, &context);

    memcpy(&hash_value, digest + 8, sizeof(uint64_t));
    return hash_value;
}
