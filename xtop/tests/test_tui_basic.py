...
async def select_single_column_in_grouping(pilot, column: str):
    ...
    # Clear any existing search first by pressing escape and reopening
    await pilot.press("escape")
    await pilot.pause(0.2)
    await pilot.press("g")
    await pilot.pause(0.5)
    
    # Type just the first 3 characters to search
    search_text = column[:3].lower()
    ...
    await pilot.pause(1.0)  # Give more time for query to execute
...
async def test_add_single_column():
    ...
    async with app.run_test(size=(120, 40)) as pilot:
        ...
        # Try to add TID column (more likely to exist)
        await select_single_column_in_grouping(pilot, 'TID')
        ...
        print(f"New columns after adding TID: {new_cols}")
...
async def test_add_multiple_columns():
    ...
    async with app.run_test(size=(120, 40)) as pilot:
        ...
        # Add second column (TGID)
        await select_single_column_in_grouping(pilot, 'TGID')
        await pilot.pause()
        await wait_for_table_load(pilot)
