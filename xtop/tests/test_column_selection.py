...
def test_unified_column_list():
    ...
    columns_by_source = {
        ...
        'kstacks': ['STACK_HASH', 'STACK_SYMS'],
        'ustacks': ['STACK_HASH', 'STACK_SYMS']
    }
    ...
    stack_hash_entries = [item for item in unified if item[0] == 'STACK_HASH']
    assert len(stack_hash_entries) == 1, "STACK_HASH should appear once"
...
def test_filter_columns_by_pattern():
    ...
    test_columns = [
        ...
        ('STACK_HASH', 'stack_hash (kstack)', 'STACK_HASH'),
        ('TID', 'tid (samples)', 'TID'),
    ...
    assert filtered[0][0] == 'STACK_HASH', "Pattern 'hash' should match STACK_HASH"
