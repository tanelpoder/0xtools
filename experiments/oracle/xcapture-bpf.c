...
// context enrichment demo: Oracle wait events

#define KSLEDT_SYM_ADDRESS   0x600069f0
#define EVENT_NAME_SLOT_SIZE 56

// begin oracle wait event
int uprobe_kskthbwt(struct pt_regs *ctx) {
    u32 tid = bpf_get_current_pid_tgid() & 0xffffffff;
    u32 pid = bpf_get_current_pid_tgid() >> 32;
    ...
    // read the wait event name from ksledt_ array using arg2 as offset
    u64 event_name_array_start;
    bpf_probe_read_user(&event_name_array_start, sizeof(event_name_array_start), (void *)KSLEDT_SYM_ADDRESS);
    
    u64 arg2 = PT_REGS_PARM2(ctx);
    u64 event_struct_addr = (u64) event_name_array_start + (EVENT_NAME_SLOT_SIZE * arg2);

    u64 event_name_ptr;
    bpf_probe_read_user(&event_name_ptr, sizeof(event_name_ptr), (void *)event_struct_addr);
    
    bpf_probe_read_user_str(t->oracle_wait_event, sizeof(t->oracle_wait_event), (void *)event_name_ptr);
...
// end oracle wait event
int uprobe_kskthewt(struct pt_regs *ctx) {
    u32 tid = bpf_get_current_pid_tgid() & 0xffffffff;
    u32 pid = bpf_get_current_pid_tgid() >> 32;
    ...
    t->tid = tid;
    t->pid = pid;
    t->oracle_wait_event[0] = '-';
    t->oracle_wait_event[1] = '\0';
